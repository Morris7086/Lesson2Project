...
public class SecondLoginTest {
    private static final String[] browserList = {Browsers.CHROME, Browsers.FIREFOX};
    ...
    void prepareForTest(TestInfo testInfo) {
        ...
        String choseBrowser = Configuration.browser = browserList[new Random().nextInt(2)];
        System.out.println("Выбран браузер " + choseBrowser);
        open("https://slqa.ru/cases/ChatGPTLogin/");
    }

    @AfterEach
    void actionAfterTest(TestInfo testInfo) {
        System.out.println("Конец теста " + testInfo.getDisplayName());
        closeWindow();
    }

    @DisplayName("1. Проверка успешного входа под несколькими пользователями")
    @ParameterizedTest
    ...
    void checkSuccessLoginSomeUsers(String username) {
        ...
        checkMsg(LOGIN_MSG, "Вход в систему выполнен успешно! Загрузка...");
    ...
    void checkWrongLoginWithWrongPass() {
        ...
        checkMsg(LOGIN_MSG, "Invalid username or password.");
    }
    ...
    void checkWrongLoginWithLockedUser() {
        ...
        $(LOGOUT_BUTTON).shouldNotBe(visible);
        checkMsg(LOGIN_MSG, "Пользователь заблокирован.");
    }
    ...
    void checkWrongLoginWithEmptyUsername() {
        ...
        $(LOGOUT_BUTTON).shouldNotBe(visible);
        checkMsg(LOGIN_MSG, "Username is required.");
    }
    ...
    void checkWrongLoginWithEmptyPass() {
        ...
        $(LOGOUT_BUTTON).shouldNotBe(visible);
        checkMsg(LOGIN_MSG, "Password is required.");
    }
    ...
    void checkWrongLoginWithEmptyUsernameAndPass() {
        ...
        $(LOGOUT_BUTTON).shouldNotBe(visible);
        checkMsg(LOGIN_MSG, "Username and Password are required.");
    }
